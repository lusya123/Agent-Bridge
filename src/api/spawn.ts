    ...
    if (targetMachine !== config.machine_id && targetMachine !== 'auto') {
      ...
      if (!machine) {
        return c.json(errorResponse(
          ...
          `Machine "${targetMachine}" not found`,
        ), 404);
      }
      try {
        ...
        try {
          ...
          return c.json(errorResponse(
            ...
            `Remote bridge "${targetMachine}" returned non-JSON response`,
            getErrorDetail(err),
          ), 502);
        }
